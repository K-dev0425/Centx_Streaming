    ...
    case "update_video":
        {
            ...
            $center_lat = isset($_POST['center_lat']) ? $_POST['center_lat'] : "";
            $center_lng = isset($_POST['center_lng']) ? $_POST['center_lng'] : "";
            $map_radius = isset($_POST['map_radius']) ? $_POST['map_radius'] : "";
            $selected_channel_names = isset($_POST['selected_channel_names']) ? $_POST['selected_channel_names'] : "";

            $channel_array = explode(",", $selected_channel_names);

            $insert_query = "";

            foreach ($channel_array as $channel_name) {

                $query = "SELECT userid FROM " . tbl("users") . " WHERE username = '" . $channel_name . "'";
                $result = db_select($query);
                $userid = $result[0]['userid'];

                $insert_query = "INSERT INTO " . tbl("sponsor_channel") . " (owner, sponsor_ad) VALUES (". (int)$userid .", ". (int)$video_id .")";
            ...
            $queryd .= "UPDATE " . tbl("video") . " SET allowed_min_age = " . (int)$allowed_min_age . ", allowed_max_age = " . (int)$allowed_max_age . ", allowed_gender = '" . $allowed_gender . "', allowed_country = '" . $allowed_country . "', center_lat = '" . $center_lat . "', center_lng = '" . $center_lng . "', map_radius = '" . $map_radius . "',
            ...
            $queryd .= ", type = '" . $type . "', pay_type = '" . $pay_type . "'  WHERE videoid = " . (int)$video_id;
            $db->Execute($queryd);
