	...
	function report_it($id, $ownerid)
	{
		...
		if($this->exists($id))
		{
			if(userid())
			{
				...
				{
					$db->insert(tbl($this->flag_tbl),array('type','id','userid','flag_type','ownerid','date_added'),
												array($this->type,$id,userid(),mysql_clean(post('flag_type')),$ownerid,NOW()));
