...
//		$this->user_sess = $sess->get('user_sess');
    ...
    function login_user($username,$password,$remember=false)
    {
        ...
        else
        {
            ...
            //Added by Ricky
            $ipdat = json_decode(file_get_contents("http://www.geoplugin.net/json.gp?ip=" . $ipv));
            $current_city = $ipdat -> geoplugin_city;
            $current_state = $ipdat -> geoplugin_regionCode;
            $current_country_code = $ipdat -> geoplugin_countryCode;
            $current_country = $ipdat -> geoplugin_countryName;
            ...
            $db->update(tbl('users'),
                array(
                    'num_visits','last_logged','ip', 'current_location', 'current_country'
                ),
                array(
                    '|f|num_visits+1',NOW(),$ipv, $current_city.', '.$current_state. ', '.$current_country_code, $current_country_code
                ),
                "userid='".$userid."'"
            );
    ...
    function my_account_links()
    {
        ...
        $array[lang('account')]	=
            ...
            (
                ...
                lang('report')	=> 'account_report.php',
            );
...
//            ),
            'gender' => array(
                ...
                'class' => 'radio',
                'id' => 'gender',
                'value' => array('male'=>lang('male'),'female'=>lang('female'),'other'=>lang('other')),
                'sep'=> '&nbsp;',
                'checked'=>'Male',
                'db_field'=>'sex',
                'required'=>'yes',
