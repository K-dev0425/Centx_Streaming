...
//    updated By Ricky
    function get_video_attachments($vid) {
        $query = "SELECT * FROM " . tbl("video_attachments") . " WHERE video_id = " . (int)$vid;
    ...
    function update_video($array=NULL)
    {
        ...
        if(empty($eh->error_list))
        {
            ...
            $price_fields = $Upload->loadPriceFields($array);
            $filtering_fields = $Upload->loadFilteringFields($array);
            $survey_fields = $Upload->loadSurveyFields($array);

            $upload_fields = array_merge($required_fields,$location_fields,$option_fields, $price_fields, $filtering_fields, $survey_fields);
...
//			".("video.active")."='yes' AND ".("video.broadcast")." !='unlisted' ";
        ...
        if ($params['type'])
        {
            ...
            $cond .= " ".("video.pay_type")." = '".$pay_type."' AND ";

            if ($type == 'ad_audio') {
                $cond .= " ".("video.type")." <> 'ad'";
            }
            else if ($type == 'audio') {
                $cond .= " "."(".("video.type")." = 'audiobook' OR ";
                $cond .= " ".("video.type")." = 'podcast'".")";
            }
            else {
                $cond .= " ".("video.type")." = '".$type."'";
            }
        ...
        if(!$params['count_only'] &&  !$params['show_related'])
        {
            ...
            $query = "SELECT * FROM ".cb_sql_table( 'video' );
            $query .= " LEFT JOIN ".cb_sql_table( 'users' )." ON video.userid = users.userid";
