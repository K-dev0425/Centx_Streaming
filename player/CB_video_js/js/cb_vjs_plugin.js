...
cbvjslogo.prototype.init = function(){
	...
	CbLogoBrand.innerHTML = '<img id="video_player_logo" src="/new_layout/3dlogo.png" alt=""><img id="video_player_ad_icon" src="/images/ad_icon.png" alt="">';
...
//Updated By Ricky
var jsoverlay = function (player, options) {
	var jsoverlay = this;

	jsoverlay.videoid = options.videoid;
	jsoverlay.player = player;

	jsoverlay.init();
}

jsoverlay.prototype.init = function () {
	var jsoverlay = this;
	var JsOverlay = document.createElement("div");
	JsOverlay.id = "vjs-cb-overlay";
	JsOverlay.className = "vjs-cb-overlay";
	JsOverlay.innerHTML = "<div id='quiz_question_1'></div><div class='quiz_answers_1 form-group'></div>";

    var BigPlayButton = cbvjsheader.player.getChild('bigPlayButton').el_;
    jsoverlay.player.el_.insertBefore(JsOverlay, BigPlayButton);
}
...
cbvjsheader.prototype.init = function(){
	...
	CbVjsHeader.innerHTML = "<div class='captionBlock'><div class='vidTitle col'><a target='_blank' href='"+baseurl+"/watch_video.php?v="+cbvjsheader.videoid+"'>"
		+cbvjsheader.title+"</a></div><div class='uploaderName col'>by "+cbvjsheader.uploader+"</div></div>";
