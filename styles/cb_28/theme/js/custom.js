...
$(document).ready(function(){
	...
	// R.K
    $('#embedding').parent().parent('.form-group').hide();

});


function homePageVideos(qlist_items) {
	$('#container').on("click","#recent_load_more, #featured_load_more, #recentads_load_more, #recentads_audio_load_more, #recentaudiobooks_load_more, #recentpodcasts_load_more", function(){
		...
		if (loadHit == 1) {
			...
			recentAdAudioSect = $('#container').find('#total_ad_audio_recent').text();
			...
			totalRecentAdAudios = recentAdAudioSect;
			...
			recentAdAudioShown = loadHit * loadHit - loadLimit;
			...
			gotMoreRecentAdAudio = parseInt(totalRecentAdAudios) - parseInt(recentAdAudioShown);
            ...
            if (gotMoreRecentAdAudio > 6) {
                recentFound = 3;
            } else {
                moreRecentAdAudios = false;
                recentFound = gotMoreRecentAdAudio;
            }
        ...
        $.ajax({
			...
			beforeSend: function() {
				...
				if (loadType != 'count') {
                    ...
                    else if (loadMode == 'recent_ad_audio') {
                        preLoadingBlock();
                        var currWidth = $(window).width();
                        if (loadHit >= 2 && currWidth > 767) {
                            var moveTo = $("#recentads_audio_pre").last().offset().top();
                            thakkiLoading(moveTo);
                        }
                    }
			...
			success: function(data) {
				...
				if (data.length < 10) {
					...
					if (loadHit == 1) {
						...
						if (loadMode == 'featured') {
							...
							$("#featured_vid_sec").html('<div class="break2"></div><span style="background: transparent; border: none" class="well well-info btn-block"><img src="/images/no_featured.png" ></span>');
							return false;
                            ...
                            $("#recent_vids_sec").html('<div class="break2"></div><span style="background: transparent; border: none" class="well well-info btn-block"><img src="/images/no_featured.png" ></span>');
							return false;
						}
						else if (loadMode == 'recent_ad') {
							for (var i = 0; i < recentFound; i++) {
                            ...
                            $("#recent_ads_sec").html('<div class="break2"></div><span style="background: transparent; border: none" class="well well-info btn-block"><img src="/images/no_featured.png" ></span>');
                            return false;
						}
                        else if (loadMode == 'recent_ad_audio') {
                            for (var i = 0; i < recentFound; i++) {
                                $(document).find('#recentads_audio_pre').append('<div class="item-video col-lg-4 col-md-4 col-sm-4 col-xs-6"><div class="thumb-video background-masker clearfix"></div><div class="loadingInfo video-info relative clearfix"><div class="background-masker heading clearfix"></div><div class="background-masker paragraph clearfix"></div><div class="background-masker clearfix views-date"></div></div></div>');
                            }
                            $('#recentads_audio_load_more').remove();
                            $('#recentads_audio_pre').remove();
                            // $("#recent_ads_audio_sec").html('<div class="break2"></div><span class="well well-info btn-block">'+noRecent+'</span>');
                            $("#recent_ads_audio_sec").html('<div class="break2"></div><span style="background: transparent; border: none" class="well well-info btn-block"><img src="/images/no_featured.png" ></span>');
                            return false;
                        }
                        else if (loadMode == 'recent_audiobook') {
                            for (var i = 0; i < recentFound; i++) {
                            ...
                            $("#recent_audiobooks_sec").html('<div class="break2"></div><span style="background: transparent; border: none" class="well well-info btn-block"><img src="/images/no_featured.png" ></span>');
                            return false;
                        }
                        else if (loadMode == 'recent_podcast') {
                            for (var i = 0; i < recentFound; i++) {
                            ...
                            $("#recent_podcasts_sec").html('<div class="break2"></div><span style="background: transparent; border: none" class="well well-info btn-block"><img src="/images/no_featured.png" ></span>');
                            return false;
                        }
					}
					return true;
                ...
                if (loadType == 'ad_audio') {
                    if (loadMode == 'recent_ad_audio') {
                        $('#recentads_audio_load_more').remove();
                        $('#recentads_audio_pre').html('');
                        $(data).appendTo('#recent_ads_audio_sec').fadeIn('slow');
                        recentAdAUdioSect = $('#container').find('#total_ad_audio_recent').text();
                        recentAdAUdioSect = parseInt(recentAdAUdioSect);
                        loadHit = parseInt(loadHit);
                        loadLimit = parseInt(loadLimit);

                        if (loadHit == 1 && loadLimit >= recentAdAUdioSect) {
                            moreRecentAdAudios = false;
                        }else if (loadHit * loadLimit >= recentAdAUdioSect) {
                            moreRecentAdAudios = false;
                        }

                        if (moreRecentAdAudios == true) {
                            $(document).find('#recentads-audio-loadmore').append('<div class="clearfix text-center"><button id="recentads_audio_load_more" class="btn btn-loadmore" loadtype="ad_audio" loadmode="recent_ad_audio" loadlimit="'+loadLimit+'" loadhit="'+newloadHit+'">'+loadMoreLang+'</button></div>');
                        }
                    }
                }
				...
				$('#container').find('#total_ad_audio_recent').hide();
			}
		});
	});
	...
	$(document).ready(function(){
		...
		$('#recentads_load_more').trigger("click");
		$('#recentads_load_more').hide();
        $('#recentads_audio_load_more').trigger("click");
