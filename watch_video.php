...
if ($user_det != '' && (int)userid() != 1 && $user_det['userid'] != $vdo['userid']) {
    ...
    if ((int)$age < (int)$min_age && (int)$min_age > 0) {
        echo "<script>alert('Only " . $age . $min_age . "+ can watch this video.');</script>";
        echo "<script>history.go(-1);</script>";
        exit();
    }
    ...
    if ($allowed_country != 'all' && $country != $allowed_country && $allowed_country != '') {
        echo "<script>alert('Only " . $allowed_country . " users can watch this video.');</script>";
        echo "<script>history.go(-1);</script>";
        exit();
    }

    if ($vdo['pay_type'] == 'ad' && $vdo['remaining_price'] != null && (float)$vdo['remaining_price'] <= (float)$vdo['price_per_sec']) {
...
//    check map radius
    if ($vdo['map_radius'] != '' && $vdo['center_lat'] != '') {
        ...
        $earthRadius = 6371000;
        $latFrom = deg2rad((float)$vdo['center_lat']);
        $lngFrom = deg2rad((float)$vdo['center_lng']);
        ...
        if ($distance > (float)$vdo['map_radius']) {
            echo "<script>alert('You can not watch this video. You are out of the allowed area.');</script>";
            echo "<script>history.go(-1);</script>";
            exit();
        }
